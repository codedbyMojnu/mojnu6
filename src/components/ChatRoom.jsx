...
export default function ChatRoom({ isOpen, onClose, roomId = 'general' }) {
  ...
  useEffect(() => {
    if (isConnected && socket && isOpen && user?.token && profile?.username) {
      ...
      console.log('Joining chat room with:', { roomId, username: profile.username, userId });
  ...
  }, [isConnected, socket, isOpen, roomId, user?.token, profile?.username, getUserId, logUserInfo]);
              ...
              <>
                {messages.length === 0 ? (
                  ...
                  messages.map((message) => (
                    <div
                      key={message._id}
                      className={`p-3 rounded-lg border ${getMessageColor(message.username)} ${getMessageTypeStyle(message.messageType)}`}
                    >
