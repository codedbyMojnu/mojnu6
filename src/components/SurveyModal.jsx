  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (err.response && err.response.status === 409) {
        ...
        setError('আপনি ইতিমধ্যে ফিডব্যাক দিয়েছেন।');
      } else {
  ...
  return (
    <div className="modal-overlay animate-fade-in z-50">
      <div className="modal-content p-6 max-w-md mx-4 animate-bounce-in relative">
        ...
        <h2 className="text-2xl font-bold text-center mb-4 text-indigo-700">📝 ফিডব্যাক / Survey</h2>
        {avgRating !== null && (
          <div className="text-center mb-2 text-sm text-gray-600">
            গড় রেটিং: <span className="font-bold text-yellow-600">{avgRating} / 5</span> ({total} জন)
          </div>
        )}
        {alreadySubmitted ? (
          <div className="text-center text-green-700 font-bold text-lg py-8">আপনি ইতিমধ্যে ফিডব্যাক দিয়েছেন।</div>
        ) : submitted ? (
          <div className="text-center text-green-700 font-bold text-lg py-8">ধন্যবাদ! আপনার মতামত গ্রহণ করা হয়েছে।</div>
        ) : (
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              <label className="block mb-2 font-semibold">১। এই এপ্লিকেশনের ডিজাইন, ইউজার এক্সপেরিয়েন্স আপনার কেমন লেগেছে?</label>
              <div className="flex items-center justify-center gap-1 mb-1">
                {[1,2,3,4,5].map(star => (
                  ...
                  >
                    <span className={`text-3xl ${star <= (hoverRating || rating) ? 'text-yellow-400' : 'text-gray-300'}`}>★</span>
                  </button>
                ))}
              </div>
              <div className="text-xs text-gray-500 text-center">{rating ? `আপনার রেটিং: ${rating}/5` : ''}</div>
            </div>
            <div>
              <label className="block mb-2 font-semibold">২। আমরা এই কুইজ এপ্লিকেশন প্লাটফর্মটি বন্ধ করে দিলে</label>
            ...
            <div>
              <label className="block mb-2 font-semibold">৩। আপনার পরামর্শ যদি থাকে যেমন নতুন ফিচার নিয়ে, কোনটি ভালো লাগে নি</label>
              <textarea
